...
class VarieteController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $actions = [
            new Action(ucwords('Ajouter nouveau'), Action::TYPE_NORMAL, url: route('stadeVarietes.create' , [
                // 'id_espece' => $model[ModelTarget::PK],
                'id_variete' => $model[ModelTarget::PK],
                'back' => url()->current()
            ])),
            new Action(ucwords('supprimer les selctionner'), Action::TYPE_DELETE_ALL, url: route('stadeVarietes.destroyGroup'))
        ];
        $heads = [
